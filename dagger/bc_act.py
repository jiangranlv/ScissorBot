...
from rl.loss_utils import HelperPointLoss, ActionChunkingLoss
...
class ActionDeTokenizer(nn.Module):
    def __init__(self, action_token_dim: int, close_head_cfg = {}, 
        ...
        self.tune_head = MLP(input_dim=action_token_dim, output_dim=9, **tune_head_cfg)
...
class ActionChunking(nn.Module):
    def __init__(self, action_token_dim: int, close_head_cfg = {}, 
                 trans_head_cfg={}, rot_head_cfg={}, tune_head_cfg={}, chunking_size = 1) -> None:
        super().__init__()

        self.head_list = nn.ModuleList([ActionDeTokenizer(action_token_dim, close_head_cfg, 
                 trans_head_cfg, rot_head_cfg, tune_head_cfg) for i in range(chunking_size)])
        
    def forward(self, action_token: torch.Tensor) -> torch.Tensor:
        x = action_token
        # B * output_dim -> B * N * output_dim
        future_action_list = torch.stack([head(x) for head in self.head_list], dim= 1)
        return future_action_list
...
class PointNetTransformerModule(nn.Module):
    def __init__(self, seq_len=6, input_statistics={}, pose_input_dim = 6,
                 ...
                 use_helper = False, no_pose = False, chunking_size = 1) -> None:
        super().__init__()
        ...
        self.model_output_dim = 20
        self.action_detokenizer = ActionChunking(action_token_dim=self.token_dim*self.seq_len, **action_detokenizer_cfg, chunking_size= chunking_size)
        self.action_loss = ActionChunkingLoss(**action_loss_cfg, translation_unit=get_translation_std_mean(input_statistics))
    ...
    def _postprocess_action_pred(self, action_pred: torch.Tensor, pose: torch.Tensor) -> torch.Tensor:
        """
        Args:
            action_pred: [?, 7]
        Output:
        ...
        # return output
        cut_direc = pose[:, 3:]
        rot_direc = torch.einsum('nab,nb->na',batch_torch_A_to_R(action_pred[:, 2:11]), cut_direc)
        tune_direc = torch.einsum('nab,nb->na',batch_torch_A_to_R(action_pred[:, 11:]), cut_direc)
        action_pred = torch.concat([action_pred[:, :2], rot_direc, tune_direc], dim= -1)
        return action_pred
    ...
    def predict(self, pc_seq: torch.Tensor, pose_seq: torch.Tensor) -> torch.Tensor:
        ac_pred, _ = self.forward(pc_seq, pose_seq)
        return ac_pred

    def forward_loss(self, ac_pred: torch.Tensor, ac_gt: torch.Tensor, 
                     hp_seq_pred: torch.Tensor, hp_seq_gt: torch.Tensor, pose:torch.Tensor,
                     reduce: bool=True) -> Tuple[torch.Tensor, dict]:
        assert ac_pred.shape[1 : ] == (self.chunking_size, self.model_output_dim) \
       and ac_gt.shape[1 : ] == (self.chunking_size, 4), f"{ac_pred.shape}, {ac_gt.shape}"

        ac_gt = self._preprocess_action_gt(ac_gt)
        action_loss, action_info = self.action_loss(ac_pred, ac_gt, pose = pose, reduce = reduce)
...
class PointNetTransformerBC(pl.LightningModule):
    ...
    def forward_loss(self, ac_pred, ac_gt, hp_seq_pred, hp_seq_gt, pose, reduce=True) -> Tuple[torch.Tensor, dict]:
        return self.model.forward_loss(ac_pred, ac_gt, hp_seq_pred, hp_seq_gt, pose, reduce)
    ...
    def training_step(self, batch, batch_idx):
        ...
        pc_seq, pose_seq, hp_seq_gt, ac_gt, data_idx, future_pose_seq = batch
        ac_pred, hp_seq_pred = self.forward(pc_seq, pose_seq)
        train_loss, loss_info = self.forward_loss(ac_pred, ac_gt, hp_seq_pred, hp_seq_gt, pose= future_pose_seq)
    ...
    def validation_step(self, batch, batch_idx):
        pc_seq, pose_seq, hp_seq_gt, ac_gt, data_idx, future_pose_seq = batch
        ac_pred, hp_seq_pred = self.forward(pc_seq, pose_seq)
        eval_loss, loss_info = self.forward_loss(ac_pred, ac_gt, hp_seq_pred, hp_seq_gt, pose= future_pose_seq)
...
def main():
    ...
    train_data, eval_data = get_dagger_dataset(
        ...
        model_kwargs["seq_len"], args.pre_cut_len, action_type_num, score_thres= args.score_thres, 
        rot_jitter = args.rot_jitter, chunking_size= model_kwargs["chunking_size"])
