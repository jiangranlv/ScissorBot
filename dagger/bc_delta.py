...
from rl.loss_utils import HelperPointLoss, ActionLossDelta
...
class PointNetTransformerModule(nn.Module):
    def __init__(self, seq_len=6, input_statistics={}, pose_input_dim = 6,
        ...
        self.action_loss = ActionLossDelta(**action_loss_cfg, translation_unit=get_translation_std_mean(input_statistics))
...
class PointNetTransformerBC(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        train_loss, loss_info = self.forward_loss(ac_pred, ac_gt, hp_seq_pred, hp_seq_gt, pose= pose_seq[:, -1, :])
    ...
    def validation_step(self, batch, batch_idx):
        pc_seq, pose_seq, hp_seq_gt, ac_gt, data_idx = batch
        ac_pred, hp_seq_pred = self.forward(pc_seq, pose_seq)
        eval_loss, loss_info = self.forward_loss(ac_pred, ac_gt, hp_seq_pred, hp_seq_gt, pose= pose_seq[:, -1, :])
