...
from rl.loss_utils import HelperPointLoss, ActionChunkingPoseLoss
...
class ActionDeTokenizer(nn.Module):
    def __init__(self, action_token_dim: int, angle_head_cfg = {}, 
                 trans_head_cfg={}, rot_head_cfg={}) -> None:
        super().__init__()

        self.angle_head = MLP(input_dim=action_token_dim, output_dim=1, **angle_head_cfg)
    ...
    def forward(self, action_token: torch.Tensor) -> torch.Tensor:
        ...
        angle = self.angle_head(x, head = True)
        trans = self.trans_head(x, head = True)
        rot = self.rotation_head(x, head = True)
        rot = rot / torch.norm(rot, dim= -1, keepdim= True)
        
        action_pred = torch.concat([angle, trans, rot], dim = -1)
        return action_pred 

class ActionChunking(nn.Module):
    def __init__(self, action_token_dim: int, angle_head_cfg = {}, 
                 trans_head_cfg={}, rot_head_cfg={}, chunking_size = 1) -> None:
        super().__init__()
        self.head_list = nn.ModuleList([ActionDeTokenizer(action_token_dim, angle_head_cfg, 
                 trans_head_cfg, rot_head_cfg) for i in range(chunking_size)])
...
class PointNetTransformerModule(nn.Module):
    def __init__(self, seq_len=6, input_statistics={}, pose_input_dim = 6,
        ...
        self.action_loss = ActionChunkingPoseLoss(**action_loss_cfg, translation_unit=get_translation_std_mean(input_statistics))
    ...
    def predict(self, pc_seq: torch.Tensor, pose_seq: torch.Tensor) -> torch.Tensor:
        ...
        return self._postprocess_action_pred(ac_pred, pose= pose_seq[:, -1, :])
    ...
    def forward_loss(self, ac_pred: torch.Tensor, ac_gt: torch.Tensor, 
       ...
       and ac_gt.shape[1 : ] == (self.chunking_size, 7), f"{ac_pred.shape}, {ac_gt.shape}"

        ac_gt = self._preprocess_action_gt(ac_gt)
        action_loss, action_info = self.action_loss(ac_pred, ac_gt, reduce = reduce)
...
def main():
    ...
    train_data, eval_data = get_dagger_dataset(
        ...
        use_angle_pose= True, chunking_size= model_kwargs["chunking_size"])
