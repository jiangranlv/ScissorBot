...
def make_single_dataset(data_path: List[str], mode: Literal["train", "eval"], seq_len: int, 
    ...
    f_pc = DataForest(pc_path, target_file_suffix=[".npy"], info_file_suffix=[], target_file_name="compressed")
...
class Vision2StateModule(nn.Module):
    def __init__(self, seq_len=6, input_statistics={},
                 ...
                 goal_predictor_cfg = {}, use_helper = False) -> None:
        super().__init__()
        ...
        self.transformer = TransformerModule(self.token_dim, PE_cfg, transformer_cfg) if seq_len> 1 else None
        # self.mlp = MLP(input_dim=self.token_dim*self.seq_len, output_dim=self.token_dim, hidden_dim= [self.token_dim, self.token_dim])
        
        self.goal_encoder = MLP(input_dim= 3, output_dim= 64, hidden_dim= [64])
        self.goal_predictor = MLP(input_dim=self.token_dim*seq_len + 64, output_dim= 6, **goal_predictor_cfg)
    ...
    def forward(self, pc_seq: torch.Tensor, pose_seq: torch.Tensor,fp: torch.Tensor, next_edge: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # sanity check
        ...
        assert fp.shape[1:] == (3,), f"(?, 3) expected, ({fp.shape}) got"
        assert next_edge.shape[1:] == (2, 3), f"(?, 3) expected, ({next_edge.shape}) got"
        B, T = pc_seq.shape[0:2]

        next_edge = self._preprocess_next_edge_gt(next_edge)
        next_a = next_edge[:, :3]
        hp_seq_pred =None 
        goal_pred = None
        ac_pred = None
        ...
        next_a_embedding = self.goal_encoder(next_a)    
        goal_pred = self.goal_predictor(torch.concat([flatten_output, next_a_embedding], dim = -1), head= True) #B, 6
        if self.goal_pred_offset:
            goal_pred += fp.repeat(1, 2)
        
        return ac_pred, hp_seq_pred, goal_pred
    
    def forward_single_frame(self, pc_seq: torch.Tensor, pose_seq: torch.Tensor, fp: torch.Tensor, next_edge: torch.Tensor):
        # sanity check
        assert pc_seq.shape[1:] == (self.seq_len, POINT_RES, POINT_DIM), f"(?, {self.seq_len}, POINT_RES, {POINT_DIM}) expected, ({pc_seq.shape}) got"
        assert pose_seq.shape[1:] == (self.seq_len, 8), f"(?, {self.seq_len}, 8) expected, ({pose_seq.shape}) got"
        assert fp.shape[1:] == (3,), f"(?, 3) expected, ({fp.shape}) got"
        assert next_edge.shape[1:] == (2, 3), f"(?, 2, 3) expected, ({next_edge.shape}) got"
        ...
        next_edge = self._preprocess_next_edge_gt(next_edge)
        next_a = next_edge[:, :3]
        pc, pose = pc_seq[:, -1, :, :], pose_seq[:, -1, :]
        pc, pose = self._preprocess_input(pc, pose)
        pc_token: torch.Tensor = self.net(pc) # [B, self.feature_dim]
        pose_token: torch.Tensor = self.pose_tokenizer(pose) # [B, self.pose_token_dim]

        next_a_embedding = self.goal_encoder(next_a)    
        goal_pred = self.goal_predictor(torch.concat([pc_token, pose_token, next_a_embedding], dim = -1), head =True)
    ...
    def forward_loss(self, ac_pred: torch.Tensor, ac_gt: torch.Tensor, 
        ...
        info = copy.deepcopy(goal_pred_info)
...
class Vision2State(pl.LightningModule):
    def __init__(self, learning_rate=1e-3, 
                 optimizer_name="Adam", optimizer_kwargs={},
                 schedule_name="ExponentialLR", schedule_kwargs={"gamma": 1.0}, seq_len= 4, **kwargs):
        super().__init__()

        self.model = Vision2StateModule(seq_len,**kwargs)
        self.seq_len = seq_len
    ...
    def forward(self, pc_seq: torch.Tensor, pose_seq: torch.Tensor, fp: torch.Tensor, next_edge: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.seq_len == 1:
            return self.model.forward_single_frame(pc_seq, pose_seq, fp, next_edge)
        else:
            return self.model.forward(pc_seq, pose_seq, fp, next_edge)
    ...
    def training_step(self, batch, batch_idx):
        ...
        ac_pred, hp_seq_pred, goal_pred = self.forward(pc_seq, pose_seq, fp, next_edge)
...
def main():
    ...
    if args.checkpoint_path is not None:
        model = Vision2State.load_from_checkpoint(
            args.checkpoint_path, **model_kwargs, **lralg_kwargs)
    else:
        model = Vision2State(**model_kwargs, **lralg_kwargs)
